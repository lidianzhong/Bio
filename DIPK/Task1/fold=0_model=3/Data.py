...
RMA_Pathway_dict = joblib.load('../Dataset/RMA_Pathway_dict.pkl')
...
GEF = []
Cells = list(RMA_Pathway_dict.keys())
for each in Cells:
    GEF.append(RMA_Pathway_dict[each])
...
def GetData(csv_path):
    ...
    for ii in range(len(Cell)):
        ...
        graph = Data(x=x, edge_index=edge_index, edge_attr=edge_attr,
                     GEF=torch.tensor(RMA_Pathway_dict[Cell[ii]], dtype=torch.float32),
